...
bool aux2Latched = false;
uint32_t aux2CurrentFreq = AUX_PWM_DEFAULT_FREQ;
uint8_t aux2CurrentDuty = 0;
bool aux2PwmAttached = false;
...
void handleAuxCommand(const String& rawArg);
...
class StrapWriteCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) override {
    ...
    } else if(v.startsWith("AUX:")) {
      int colon = v.indexOf(':');
      handleAuxCommand(colon >= 0 ? v.substring(colon + 1) : "");
    } else if(v.startsWith("MOSFET:")) {
      int colon = v.indexOf(':');
      handleAuxCommand(colon >= 0 ? v.substring(colon + 1) : "");
    } else if(v.startsWith("AUX2:")) {
      int colon = v.indexOf(':');
      handleAux2Command(colon >= 0 ? v.substring(colon + 1) : "");
    } else if(v.startsWith("MOSFET2:")) {
      int colon = v.indexOf(':');
      handleAux2Command(colon >= 0 ? v.substring(colon + 1) : "");
...
void configureAuxPwm(uint32_t freqHz) {
  ...
  analogWriteResolution(AUX_PIN, AUX_PWM_RES_BITS);
...
void configureAux2Pwm(uint32_t freqHz) {
  uint32_t clamped = freqHz;
  if (clamped < AUX_PWM_MIN_FREQ) clamped = AUX_PWM_MIN_FREQ;
  if (clamped > AUX_PWM_MAX_FREQ) clamped = AUX_PWM_MAX_FREQ;

  if (!aux2PwmAttached || clamped != aux2CurrentFreq) {
#if defined(ARDUINO_ARCH_ESP32)
    analogWriteResolution(AUX2_PIN, AUX_PWM_RES_BITS);
    analogWriteFrequency(AUX2_PIN, clamped);
#endif
    aux2CurrentFreq = clamped;
    aux2PwmAttached = true;
  }
}
...
void applyAux2DutyPercent(uint8_t dutyPercent) {
  if (!aux2PwmAttached) {
    configureAux2Pwm(aux2CurrentFreq);
  }

  uint8_t bounded = dutyPercent > 100 ? 100 : dutyPercent;
  aux2CurrentDuty = bounded;
  ...
  analogWrite(AUX2_PIN, level);
#else
  const bool drive = AUX2_ACTIVE_HIGH ? (bounded > 0) : (bounded == 0);
  digitalWrite(AUX2_PIN, drive ? HIGH : LOW);
#endif

  aux2Latched = bounded > 0;
}
...
void setAux2Output(bool enabled) {
  configureAux2Pwm(aux2CurrentFreq);
  applyAux2DutyPercent(enabled ? 100 : 0);
}
...
bool isSafeGpio(int pinNumber) {
  ...
  if (pinNumber == SDA_PIN || pinNumber == SCL_PIN || pinNumber == HALL_ADC_PIN ||
    pinNumber == BUZZER_PIN || pinNumber == AUX_PIN || pinNumber == AUX2_PIN) {
    return false;
  }
  return true;
}
...
void handleAuxCommand(const String& rawArg) {
void handleAux2Command(const String& rawArg);
  String arg = rawArg;
  arg.trim();
  String upper = arg;
  upper.toUpperCase();
...
void applyAux2DutyPercent(uint8_t dutyPercent);
    setAuxOutput(true);
    snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX=%s", auxLatched ? "ON" : "OFF");
  } else if (upper == "OFF") {
    setAuxOutput(false);
    snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX=%s", auxLatched ? "ON" : "OFF");
...
void handleAux2Command(const String& rawArg) {
  ...
  if (upper == "ON") {
    setAux2Output(true);
    snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX2=%s", aux2Latched ? "ON" : "OFF");
  } else if (upper == "OFF") {
    setAux2Output(false);
    snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX2=%s", aux2Latched ? "ON" : "OFF");
  } else if (upper.startsWith("PULSE")) {
    ...
    delay(duration);
    setAux2Output(false);
    snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX2=PULSE,%lu", static_cast<unsigned long>(duration));
  } else if (upper.startsWith("PWM")) {
    int first = arg.indexOf(':');
    if (first < 0) {
      snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX2=ERR,PWM");
      return;
    }
    String payload = arg.substring(first + 1);
    payload.trim();
    ...
    configureAux2Pwm(freq);
    applyAux2DutyPercent(duty);
    snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX2=PWM,%lu,%u",
             static_cast<unsigned long>(aux2CurrentFreq), aux2CurrentDuty);
  } else {
    snprintf(g_notifyBuf, sizeof(g_notifyBuf), "RESP:AUX2=ERR");
  }
}
...
void startCalibration(){
  ...
  if(observed_diff < 30){ Serial.println("[WARN] diff too small -> manual adjust"); calib_done=false; }
  else {
    THRESH_CLOSE_COUNTS = max<uint16_t>(20,(uint16_t)(observed_diff*0.60f));
    THRESH_OPEN_COUNTS  = max<uint16_t>(10,(uint16_t)(observed_diff*0.30f));
    calib_done=true;
...
void setup() {
  ...
  digitalWrite(AUX2_PIN, AUX2_ACTIVE_HIGH ? LOW : HIGH);
  configureAux2Pwm(AUX_PWM_DEFAULT_FREQ);
  applyAux2DutyPercent(0);
...
void loop() {
  ...
  Serial.print(" AUX14 = "); Serial.print(auxLatched);
  Serial.print(" AUX16 = "); Serial.print(aux2Latched);
